...
class ControllerCommonFileManager extends Controller {
 ...
 public function image() {
  ...
  if (isset($this->request->get['image'])) {
   $this->response->setOutput(image_resize(html_entity_decode($this->request->get['image'], ENT_QUOTES, 'UTF-8'), 100, 100), $this->config->get('config_compression'));
  }
 }
   ...
   foreach ($files as $file) {
    ...
    if (in_array(strtolower($ext), $allowed)) {
     ...
     $json[] = array(
      ...
      'thumb'    => image_resize(utf8_substr($file, strlen(DIR_IMAGE)), 100, 100)
     );
    }
   }
  }
 ...
 public function copy() {
  ...
  if (isset($this->request->post['path']) && isset($this->request->post['name'])) {
   if ((utf8_strlen($this->request->post['name']) < 3) || (utf8_strlen($this->request->post['name']) > 255) || !$this->check_filename(utf8_strlen($this->request->post['name']))) {
 ...
 public function folders() {
  $this->response->setOutput($this->recursiveFolders(DIR_IMAGE . 'data/'), $this->config->get('config_compression')); 
 }
 ...
 public function upload() {
  ...
  if (isset($this->request->post['directory'])) {
   if (isset($this->request->files['image']) && $this->request->files['image']['tmp_name']) {
    $filename = trim(basename(html_entity_decode($this->request->files['image']['name'], ENT_QUOTES, 'UTF-8')));
    
    if ((strlen($filename) < 3) || (strlen($filename) > 255) || !$this->check_filename($filename)) {
     $json['error'] = $this->language->get('error_filename');
    }
 ...
 // 121104 Aditional requrements Begin
 protected function check_filename($filename){
 ...
 // 121104 Aditional requrements End
} 
?>