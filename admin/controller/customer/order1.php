		...
		if ($order_info) {
			...
			foreach ($results as $result) {
				$this->data['historys'][] = array(
					'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
					'status'     => $result['status'],
                    // 100203 ALNAUA E-mail comment templates for order status Begin
                    // 'comment'    => nl2br($result['comment']),
					'comment'    => html_entity_decode(nl2br($result['comment']), ENT_QUOTES, 'UTF-8'),
                    // 100203 ALNAUA E-mail comment templates for order status End
...
//		$this->data['website'] = trim(HTTP_CATALOG, '/');
    	...
    	foreach ($products as $product) {
        	...
        	$this->data['products'][] = array(
          		...
          		'price'    => $this->currency->format($product['price']-($product['price']*0.2), $order_info['currency'], $order_info['value'], false),
				'total'    => $this->currency->format($product['total']-($product['total']*0.2), $order_info['currency'], $order_info['value'], false)
        	);
    	}
