        ...
        // (+) ALNAUA 091114 (FINISH)
            if (isset($data['notify'])) {
                ...
                if ($query->num_rows) {
                    ...
                    $message .= $language->get('zakaz_nds') . "\n";
                    $message .= html_entity_decode(HTTP_CATALOG . 'index.php?route=account/invoice_nds&order_id=' . $order_id . '&secret_code=' .$query->row['secret_code'], ENT_QUOTES, 'UTF-8') . "\n\n";
                    $message .= $language->get('zakaz_no_nds') . "\n";
                    $message .= html_entity_decode(HTTP_CATALOG . 'index.php?route=account/invoice&order_id=' . $order_id . '&secret_code=' .$query->row['secret_code'], ENT_QUOTES, 'UTF-8') . "\n\n";
                    ...
                    // (+) ALNAUA 091114 (START)
                          ...
                          $result = swift_send(
                                  ...
                                  array($query->row['email'] => html_entity_decode($query->row['firstname'], ENT_QUOTES, 'UTF-8')), //to
                                  $subject, //subject
                                  $message,  //message
...
//                        $result = $mailer->send($message);
 ...
 public function getOrders($data = array()) {
  //$sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS name, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->language->getId() . "') AS status, o.date_added, o.total, o.currency, o.value FROM `" . DB_PREFIX . "order` o";
        $sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname, o.payment_company) AS name, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->language->getId() . "') AS status, o.date_added, o.total, o.currency, o.value FROM `" . DB_PREFIX . "order` o";
  ...
  if (isset($data['filter_name']) && !is_null($data['filter_name'])) {
   //$sql .= " AND CONCAT(o.firstname, ' ', o.lastname) LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
            $sql .= " AND CONCAT(o.firstname, ' ', o.lastname, o.payment_company) LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
  }
