...
class ModelAccountOrder extends Model {
 public function getOrder($order_id) {
  ...
  // (+) ALNAUA 091114 (START)
  public function getOrderBySecretCode($order_id, $secret_code) {
    $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$order_id . "' AND secret_code = '".$this->db->escape($secret_code)."' AND order_status_id > '0'");
 ...
 public function getOrderShipment($order_id, $shipment_sort_order) {
  $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_total WHERE order_id = '" . (int)$order_id . "' AND sort_order = '" . (int)$shipment_sort_order . "'");
